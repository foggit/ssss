...
)
{
    ...
    $upload_path = $default_dir . '/' . $file_name;

    // Log info untuk debugging
    error_log("File upload attempt: Name={$file_name}, Size={$file_size}, Temp={$temp_file}");
    ...
    if ($file_size <= 0 || filesize($temp_file) <= 0) {
        $upload_message = "Empty or invalid file detected (0KB).";
        echo "<p>{$upload_message}</p>";
        return;
    }
    ...
    if (file_exists($upload_path)) {
        if (!unlink($upload_path)) {
            $upload_message = "Failed to delete existing file.";
            echo "<p>{$upload_message}</p>";
            return;
        } else {
            error_log("Existing file deleted: {$upload_path}");
        }
    }
    ...
    // Verifikasi file berhasil dipindah dan tidak kosong
    if (!file_exists($upload_path) || filesize($upload_path) <= 0) {
        if (file_exists($upload_path)) unlink($upload_path);
        $upload_message = "Upload failed: Empty file detected after transfer.";
        echo "<p>{$upload_message}</p>";
        return;
    }

    // Upload sukses
    $file_url = htmlspecialchars($file_name, ENT_QUOTES, 'UTF-8');
